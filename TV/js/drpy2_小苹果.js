...
let kurl = kdata.urls.map((it) => { return `${it.key}$http://c.xpgtv.net/m3u8/${it.url}.m3u8` }).join('#');
VOD = {
vod_id: input,
vod_name: kdata.name,
vod_pic: kdata.pic,
...
input = {parse: 0, url: kurl, header: kheaders}
}),
proxy_rule: $js.toString(() => { 
let data = fetch(input.url,{headers: kheaders});
